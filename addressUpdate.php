...
<?php
require_once 'C:\wamp64\www\ArtDomain\addressModel.php';
$addressModel = new addressModel();
if(isset($_POST['saveButton'])){
	$addressModel->update($_POST);
	header('Location:addressIndex.php');
}
$result = $addressModel->find($_GET['addressId']);
$address = json_decode($result['response']);
?>
...
<form method="post">
	<table>
		<tr>
			<td>Address Id</td>
			<td><input type="number" name="addressId" required="required" 
			value="<?php echo $address->addressId; ?>" autocomplete="off"/></td>
		</tr>
		<tr>
			<td>Street Number</td>
			<td><input type="text" name="streetNumber" required="required"
			value="<?php echo $address->streetNumber; ?>" autocomplete="off"/></td>
		</tr>
		<tr>
			<td>Street Name</td>
			<td><input type="text" name="streetName" required="required" 
			value="<?php echo $address->streetName; ?>" autocomplete="off"/>
			</td>
		</tr>
		<tr>
			<td>Street Type</td>
			<td><input type="text" name="streetType" required="required"
			value="<?php echo $address->streetType; ?>" autocomplete="off"/></td>
		</tr>
		<tr>
			<td>&nbsp;</td>
